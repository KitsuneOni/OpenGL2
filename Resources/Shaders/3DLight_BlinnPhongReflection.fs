...
void main()
{
	...
	vec4 Pixel = texture2D(ReflectionMap, FragTexCoords);
	FinalColor = vec4(LightOutput, 1.0f) * mix(texture(ImageTexture0, FragTexCoords), texture(Skybox, Reflection), Pixel.r);//vec4(LightOutput, 1.0f) * texture(ImageTexture0, FragTexCoords);
}
