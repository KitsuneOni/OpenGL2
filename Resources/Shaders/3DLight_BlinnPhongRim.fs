...
vec3 CalculateLight_Point(PointLight pLight)
{
	...
	vec3 Specular = pLight.SpecularStrength * SpecularReflectivity * pLight.Color;

	// Rim Component


	float RimFactor = 1.0f - dot(Normal, ReverseViewDirection);
    RimFactor = smoothstep(0.0f, 1.0f, RimFactor);
	RimFactor = pow(RimFactor, RimExponent);
    vec3 Rim = RimFactor * RimColor * pLight.Color;
...
void main()
{
	...
	vec4 Light = vec4(LightOutput, 1.0f);
